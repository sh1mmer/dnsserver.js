...
var dnsserver = function() {
    ...
    var processPacket = function(msg, rinfo) {

        data.msg = msg;
        data.rinfo = rinfo;
       
        data.events.emit('preMsgProcess', msg, rinfo); 
        ...
        data.events.emit('postMsgProcess', q);
    };
    ...
    var createResponse = function(query, results) {
    ...
    socket.on('message', processRequest); 

    events.on('postMsgProcess', createResponse);

    events.on('postResProcess', function() {
